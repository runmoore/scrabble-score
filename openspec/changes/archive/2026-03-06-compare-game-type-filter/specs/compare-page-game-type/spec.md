...
The loader SHALL return `availableGameTypes` — the distinct game types present across all unfiltered relevant games — so the UI can render filter pills regardless of the active filter.
...
#### Scenario: Available game types derived from unfiltered games

- **WHEN** a game type filter is active
- **THEN** the loader SHALL still return all distinct game types from the unfiltered game set so that all filter pills remain visible
